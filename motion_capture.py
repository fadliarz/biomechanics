...
RMSE = 2  # tolerable error
rc = np.array([[0, 0], [160, 0], [160, 160], [0, 160]])
rc2 = np.zeros((4, 2))
ic = np.zeros((4, 2))
ic2 = np.zeros((4, 2))
L = np.zeros(8)
...
markers = {
    "first_end": {
        "lower_bound": np.array([162, 179, 203]),
        "upper_bound": np.array([182, 199, 283]),
    },
    "middle": {
        "lower_bound": np.array([17, 227, 211]),
        "upper_bound": np.array([37, 247, 291]),
    },
    "last_end": {
        "lower_bound": np.array([83, 128, 210]),
        "upper_bound": np.array([103, 148, 290]),
    },
}
...
def main():
    ...
    while True:
        ...
        # middle angle
        angle = find_angle(
            reconstruct(L, xf_medium, yf_medium),
            reconstruct(L, xm_medium, ym_medium),
            reconstruct(L, xl_medium, yl_medium),
        )
